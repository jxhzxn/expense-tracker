...
import { Expense, Category, ACCOUNTS, AccountType } from "@/lib/types";
...
export default function ExpenseForm({ expense, onSave, onClose }: Props) {
  ...
  const [category, setCategory]     = useState<Category>(expense?.category ?? categories[0] ?? "");
  ...
  function handleManageClose() {
    ...
    if (!updated.includes(category)) setCategory(updated[0] ?? "");
    setShowManage(false);
  }
  ...
  return (
    <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-sm font-medium" style={{ color: "var(--c-t2)" }}>Category</label>
                  <button
                    type="button"
      ...
      {showManage && <ManageCategoriesModal onClose={handleManageClose} />}
    </>
  );
}
